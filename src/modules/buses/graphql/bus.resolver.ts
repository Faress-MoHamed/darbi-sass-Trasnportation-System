...
export const busResolvers = {
	Query: {
		...
		buses: async (_: any, { filters, pagination }: any, context: Context) => {
			const service = new BusService(context.prisma);
			return service.listBuses(pagination);
		},
	...
	Mutation: {
		...
		updateBusStatus: async (
			_: any,
			{
				...
				status,
				maintenanceStatus,
			}: { id: string; status: BusStatus; maintenanceStatus?: string },
			context: Context
		) => {
			const service = new BusService(context.prisma);
		...
		// Delete a bus
		deleteBus: async (_: any, { id }: { id: string }, context: Context) => {
			const service = new BusService(context.prisma);
