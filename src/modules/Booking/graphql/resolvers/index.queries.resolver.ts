...
export const bookingQueries = {
	booking: protectedTenantResolver(async (_: any, args: { id: string }, ctx) => {
		const bookingService = new BookingService(ctx.prisma);

		return bookingService.getBooking(args.id, ctx.tenant?.tenantId);
	}),

	bookings: protectedTenantResolver(async (_: any, args: { input: any }, ctx) => {
		const bookingService = new BookingService(ctx.prisma);
	...
	userBookings: protectedTenantResolver(
		async (
			...
			return bookingService.getUserBookings(args.userId, ctx.tenant?.tenantId, {
				status: args.status,
				page: args.page,
				limit: args.limit,
			});
	...
	availableSeats: protectedTenantResolver(
		async (_: any, args: { tripId: string }, ctx) => {
			...
			return bookingService.getAvailableSeats(args.tripId, ctx.tenant?.tenantId);
		}
	),
	...
	verifyTicket: protectedTenantResolver(
		async (_: any, args: { ticketNumber: string }, ctx) => {
			...
			return bookingService.verifyTicket(args.ticketNumber, ctx.tenant?.tenantId);
		}
	),
};
